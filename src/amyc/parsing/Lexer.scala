  ...
  private def lexFile(ctx: Context)(f: File): Stream[Token] = {
    ...
    def nextToken(stream: Stream[Input]): (Token, Stream[Input]) = {
      ...
      if (Character.isWhitespace(currentChar)) {
        ...
        nextToken(stream.dropWhile { case (c, p) => c != '\n' && c != EndOfFile })
      }
      else if (currentChar == '/' && nextChar == '*') {
        // Multi-line comment

        if(stream.drop(2).isEmpty) ctx.reporter.fatal("unclosed comment")

        def drop(s : Stream[Input], b: Boolean, ch1: Character, ch2: Character): Stream[Input] = {
          if(s.isEmpty) ctx.reporter.fatal("unclosed comment")

          else if(ch1=='*' && ch2=='/' && b ) s.drop(2)

          else { if(s.tail.drop(1).isEmpty) ctx.reporter.fatal("unclosed comment")
            drop(s.tail, true, ch2, s.tail.tail.head._1)}
        }

        nextToken(drop(stream.tail, false, stream.tail.head._1, stream.tail.tail.head._1))
    ...
    def readToken(stream: Stream[Input]): (Token, Stream[Input]) = {
      ...
      currentChar match {
        ...
        case _ if Character.isLetter(currentChar) =>
          ...
          keywords(word) match {
            case None => (useOne(ID(word))._1, stream.drop(word.length))
            case Some(token) => (useOne(token)._1, afterWord)
          }
        ...
        case _ if Character.isDigit(currentChar) =>
          ...
          val (chiffres, suite) = stream.span { case (ch, _) => Character.isDigit(ch)}

          val nombre = BigInt(chiffres.map(_._1).mkString)

          if(!nombre.isValidInt) ctx.reporter.fatal("number more than 32 bits")

          (useOne(INTLIT(nombre.toInt))._1, suite)


        // String literal
        case '"' =>
          if(stream.drop(1).isEmpty) ctx.reporter.fatal("Unclosed string")

          def spaning(s1: StringBuffer, s2: Stream[Input]): (StringBuffer, Stream[Input]) ={
            if(s2.head._1 == '\n') ctx.reporter.fatal("\n in the string")

            else if(s2.head._1 == '"') (s1, s2)

            else{
              if(s2.tail.isEmpty) ctx.reporter.fatal("Unclosed string")
              else spaning(s1.append(s2.head._1), s2.tail)
            }

          }

          val b = new StringBuffer()
          val (wordLetters, afterWord) = spaning(b, stream.drop(1))

          val word = wordLetters.toString


          (useOne(STRINGLIT(word))._1, afterWord.drop(1))
        ...
        case _ => {ctx.reporter.fatal("wrong syntax")
          useOne(BAD())}
